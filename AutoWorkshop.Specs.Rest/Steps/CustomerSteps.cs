namespace AutoWorkshop.Specs.Rest.Steps
{
    ...
    public class CustomerSteps
    {
        ...
        private CustomerInfo _customerInput;
        ...
        public void WhenICreateANewCustomerResourceWithTheFollowingDetailsViaRest(Table table)
        {
            ...
            _customerInput = new CustomerInfo
            {
                Title = values["Title"],
                Name = values["Name"],
            ...
            _response = _actor.Calls(Post.Body(_customerInput).To("api/customer"));
        }
        ...
        public void ThenIShouldReceiveAnHttpOkayResponse()
        {
            _response.Should().NotBeNull();
        ...
        {
            _customerInput.Should().NotBeNull();

            CustomerInfo storedCustomer = _actor.AsksFor(StoredCustomer.WithName(_customerInput.Name));

            storedCustomer.Title.Should().Be(_customerInput.Title);
            storedCustomer.Name.Should().Be(_customerInput.Name);
            storedCustomer.AddressLine1.Should().Be(_customerInput.AddressLine1);
            storedCustomer.AddressLine2.Should().Be(_customerInput.AddressLine2);
            storedCustomer.AddressLine3.Should().Be(_customerInput.AddressLine3);
            storedCustomer.Postcode.Should().Be(_customerInput.Postcode);
            storedCustomer.HomePhone.Should().Be(_customerInput.HomePhone);
            storedCustomer.Mobile.Should().Be(_customerInput.Mobile);
        }
